...
/**
 * 打印所有属性
 *  Android N 之间所有属性是在/dev/__properties__文件中
 *  Android N 上，每个 security context 对应一个文件，security context 和属性前缀对应关系保存在/property_contexts 文件中
 */
void dump_all() {
    if (get_sdk_version() < ANDROID_N) {
...
static void initialize_property_contexts() {
    if (g_use_file || !g_info.is_valid()) {
        ...
        if (get_sdk_version() >= ANDROID_O) {
            if (access("/system/etc/selinux/plat_property_contexts", R_OK) != -1) {
                ...
                initialize_contexts("/vendor/etc/selinux/vendor_property_contexts");
                initialize_contexts("/product/etc/selinux/product_property_contexts");
                initialize_contexts("/odm/etc/selinux/odm_property_contexts");
                initialize_contexts("/system_ext/etc/selinux/system_ext_property_contexts");
            } else {
 ...
 */
int native_getprop(const char *prop_name, char *buffer, size_t buffer_size) {
    if (prop_name == NULL || buffer == NULL || buffer_size == 0) {
        return -1;
    }
    ...
    prop_info *p_info = find_prop_info(p_area, prop_name, false);
    if (p_info != NULL) {
        strncpy(buffer, p_info->value, buffer_size - 1);
...
int native_setprop(const char *prop_name, const char *prop_value) {
    ...
    // 检查 root 权限
    if (geteuid() != 0) {
        fprintf(stderr, "set property requires root\n");
        return -1;
    }
    ...
    prop_area *p_area = NULL;
    if (get_sdk_version() < ANDROID_N) {
        p_area = map_prop_area(PROPERTIES_FILE, true);
    } else {
        char context_file[128] = PROPERTIES_FILE;
        strcat(context_file, "/");
        if (g_use_file) {
        ...
        p_area = map_prop_area(context_file, true);
    }
    ...
    prop_info *p_info = find_prop_info(p_area, prop_name, true);
    if (p_info != NULL) {
        uint8_t valuelen = strlen(prop_value);
        memcpy(p_info->value, prop_value, valuelen);
        p_info->value[valuelen] = '\0';
